...
let sBox0 = sBox0Const.slice();
let sBox1 = sBox1Const.slice();
let sBox2 = sBox2Const.slice();
let sBox3 = sBox3Const.slice();
let pArray = pArrayConst.slice();

function encrypt(string) {
  string = utf8Decode(string);
  let blocks = Math.ceil(string.length / 8);
...
function decrypt(string) {
  ...
  decryptedString = utf8Encode(decryptedString);
...
function F(xL) {
  ...
  let res = addMod32(sBox0[a], sBox1[b]);
  res = xor(res, sBox2[c]);
  res = addMod32(res, sBox3[d]);
  return res;
}

function encipher(xL, xR) {
  let tmp;
  for (let i = 0; i < 16; i++) {
    xL = xor(xL, pArray[i]);
    xR = xor(F(xL), xR);
...
function generateSubkeys(key) {
  ...
  let block64 = [0, 0];
  for (i = 0; i < 18; i += 2) {
    block64 = encipher(block64[0], block64[1]);
    pArray[i] = block64[0];
    pArray[i + 1] = block64[1];
  }
...
function split64by32(block64) {
  let xL = block64.substring(0, 4);
  let xR = block64.substring(4, 8);
